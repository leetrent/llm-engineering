...
def get_secretary() -> Secretary:
    "Lazily create and cache one Secretary instance for the process."
    return Secretary()

# Prevent overlapping generations on the single model instance
SECRETARY_LOCK = Lock()
...
def process(mp3_path: str):
    try:
        ...
        # 1) Transcribe (status first so UI is responsive)
        ...
        sec = get_secretary()
        with SECRETARY_LOCK:
            # Reset context per request (in case the instance is reused)
            if hasattr(sec, "_init_messages"):
                sec._init_messages()
            for chunk in sec.stream_minutes(user_prompt):
